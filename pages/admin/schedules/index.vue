  ...
  <div class="space-y-8 max-w-7xl mx-auto">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-zinc-800 pb-6">
      <div class="space-y-1">
        ...
        <p class="text-xs sm:text-sm text-zinc-400">
          Create, update, and manage weekly church services, youth fellowships, and special prayer gatherings.
        </p>
      </div>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
      <UiCard variant="default" padding="sm" class="bg-zinc-950/80 border-zinc-800/80">
        <p class="text-[10px] font-pixel text-zinc-400 uppercase tracking-wider">TOTAL SCHEDULES</p>
        <div class="flex items-baseline justify-between mt-1">
          <span class="text-2xl sm:text-3xl font-bold text-zinc-100">{{ schedulesList.length }}</span>
          <UiBadge variant="outline" size="sm">Database</UiBadge>
        </div>
      </UiCard>

      <UiCard variant="default" padding="sm" class="bg-zinc-950/80 border-zinc-800/80">
        <p class="text-[10px] font-pixel text-emerald-400 uppercase tracking-wider">ACTIVE (LIVE)</p>
        <div class="flex items-baseline justify-between mt-1">
          <span class="text-2xl sm:text-3xl font-bold text-emerald-400">{{ activeCount }}</span>
          <UiBadge variant="success" size="sm" dot>Published</UiBadge>
        </div>
      </UiCard>

      <UiCard variant="default" padding="sm" class="bg-zinc-950/80 border-zinc-800/80">
        <p class="text-[10px] font-pixel text-zinc-400 uppercase tracking-wider">INACTIVE / DRAFTS</p>
        <div class="flex items-baseline justify-between mt-1">
          <span class="text-2xl sm:text-3xl font-bold text-zinc-400">{{ inactiveCount }}</span>
          <UiBadge variant="secondary" size="sm">Archived</UiBadge>
        </div>
      </UiCard>

      <UiCard variant="default" padding="sm" class="bg-zinc-950/80 border-zinc-800/80">
        <p class="text-[10px] font-pixel text-brand-yellow uppercase tracking-wider">PUBLIC VIEW</p>
        <div class="flex items-baseline justify-between mt-1">
          <NuxtLink to="/schedule" target="_blank" class="text-xs text-brand-yellow hover:underline flex items-center gap-1 font-medium mt-1">
            <span>Open /schedule</span>
            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" />
            </svg>
          </NuxtLink>
          <UiBadge variant="accent" size="sm">Preview</UiBadge>
        </div>
      </UiCard>
    </div>

    <!-- 3. Notification Toast / Alert -->
    <div
      v-if="toastMessage"
      :class="[
    ...
    >
      ...
      <h3 class="text-sm font-semibold text-zinc-200">No Gathering Schedules Recorded</h3>
      <p class="text-xs text-zinc-500 mt-1 max-w-sm mx-auto">
        Start by adding your first weekly youth service, sunday main, or prayer gathering.
      </p>
      <UiButton
        variant="pixel"
        size="sm"
      ...
      <div class="w-full max-w-xl bg-zinc-950 border border-zinc-800 rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div class="px-6 py-4 border-b border-zinc-800 flex items-center justify-between bg-zinc-900/50">
          <div class="flex items-center gap-2">
            <span class="font-pixel text-xs text-brand-yellow">
              {{ isEditMode ? 'EDIT SCHEDULE ENTRY' : 'ADD NEW GATHERING SCHEDULE' }}
            </span>
        ...
        <form class="p-6 space-y-4 overflow-y-auto flex-1" @submit.prevent="saveSchedule">
          ...
          <UiInput
            ...
            placeholder="e.g. Youth Saturday Night Service"
            required
            hint="The primary headline displayed to attendees"
          />
          ...
          <UiInput
            ...
            required
            hint="Room, hall, or physical venue address"
          />

          <!-- Theme Field (Optional) -->
          <UiInput
            v-model="form.theme"
            label="Monthly / Service Theme (Optional)"
            placeholder="e.g. Living with Kingdom Boldness"
            hint="Spiritual theme or series tagline"
          />
            ...
            <label class="flex items-start gap-3 p-3.5 rounded-xl border border-zinc-800 bg-zinc-900/50 cursor-pointer hover:border-zinc-700 transition-colors">
              ...
              <div class="space-y-0.5">
                <span class="text-xs font-semibold text-zinc-100 flex items-center gap-1.5">
                  Publish to Website (isActive)
                  <UiBadge :variant="form.isActive ? 'success' : 'secondary'" size="sm">
                    {{ form.isActive ? 'Active' : 'Draft' }}
                  </UiBadge>
                </span>
                <p class="text-[11px] text-zinc-400">
                  When enabled, this gathering is visible to the public on the /schedule page and home landing.
                </p>
              </div>
            </label>
          </div>
    ...
    >
      <div class="w-full max-w-md bg-zinc-950 border border-zinc-800 rounded-2xl shadow-2xl overflow-hidden p-6 space-y-4">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h3 class="text-sm font-bold text-zinc-100 font-sans">Delete Gathering Schedule?</h3>
        ...
        <p class="text-xs text-zinc-300 leading-relaxed">
          This schedule will be permanently removed from the database and will no longer be visible on the public schedules page.
        </p>
...
useHead({
  title: 'Gathering Schedules | Admin Control Center'
})
...
const saveSchedule = async () => {
  ...
  try {
    if (isEditMode.value && currentScheduleId.value) {
      ...
      closeModal()
      showToast('Gathering schedule updated successfully.')
    } else {
      // POST /api/schedules
